...
const Filter = () => {
  ...
  // To make the Price stay or active after reloading the page
  useEffect(() => {
  ...
  const handleCategoryClick = (checkbox) => {
    ...
    if (checkbox.checked === false) {
      ...
      //Set new filter query
      if (searchParams.has(checkbox.name)) {
        searchParams.set(checkbox.name, checkbox.value);
      } else {
        //Append new filter query
  ...
  // To make the checkbox stay or active after reloading the page
  ...
  return (
    <div className="border p-3 filter">
      ...
      <form id="filter_form" className="px-2" onSubmit={handleClick}>
        <div className="row">
          <div className="col">
            <input
              type="text"
